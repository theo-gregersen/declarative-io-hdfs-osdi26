...
public class DatanodeManager {
  ...
  private final long blockSizeMB;
  private int current30s;
  private long maxReconstructionMBs;
  private long currentReconstructionMB;
  ...
  DatanodeManager(final BlockManager blockManager, final Namesystem namesystem,
    ...
    this.current30s = 0; // 0 if 0-30s, 1 if 30-60s
    this.maxReconstructionMBs = conf.getLong(
        DFSConfigKeys.DECLARATIVE_RECONSTRUCTION_LIMIT_MBS_KEY,
        DFSConfigKeys.DECLARATIVE_RECONSTRUCTION_LIMIT_MBS_DEFAULT
    );
    this.blockSizeMB = conf.getLong(
        DFSConfigKeys.DFS_BLOCK_SIZE_KEY,
        DFSConfigKeys.DFS_BLOCK_SIZE_DEFAULT
    ) / 1024 / 1024;
    this.currentReconstructionMB = 0; // Loose estimate based on average blockSizeMB
  }
  ...
  public DatanodeCommand[] handleHeartbeat(DatanodeRegistration nodeReg,
    ...
    if (current30s == 0 && System.currentTimeMillis() % 60000 > 29999) {
      currentReconstructionMB = 0;
      current30s = 1;
    } else if (current30s == 1 && System.currentTimeMillis() % 60000 < 30000) {
      current30s = 0;
      currentReconstructionMB = 0;
    }
    ...
    if (totalBlocks > 0 && currentReconstructionMB / 30 < maxReconstructionMBs) {
      ...
      numReplicationTasks = Math.min(numReplicationTasks, (int)((maxReconstructionMBs * 30 - currentReconstructionMB) / blockSizeMB));
      currentReconstructionMB += numReplicationTasks * blockSizeMB;
      numECTasks = Math.min(numECTasks, (int)((maxReconstructionMBs * 30 - currentReconstructionMB) / blockSizeMB));
      currentReconstructionMB += numECTasks * blockSizeMB;
