...
public class RouterClientProtocol implements ClientProtocol {
  ...
  public void declareIO(String ioDeclarationType, String src, long offset, long length, String deadline)
    ...
    // Using RemoteParam() for src path because it might change depending on namespace/relative path?
    RemoteMethod remoteMethod = new RemoteMethod("declareIO",
        new Class<?>[] {String.class, String.class, long.class, long.class, String.class},
        ioDeclarationType, new RemoteParam(), offset, length, deadline);

    List<RemoteLocation> locations =
        rpcServer.getLocationsForPath(src, false, false);
    rpcClient.invokeSequential(locations, remoteMethod, long.class, null);
  }

  public static final Logger LOG2 =
  ...
  public List<Long> declareIOs(String ioDeclarationType, List<String> paths, List<Long> offsets, List<Long> lengths, long segmentsNeeded, String deadline)
    ...
    RemoteMethod remoteMethod = new RemoteMethod("declareIOs",
        new Class<?>[] {String.class, List.class, List.class, List.class, long.class, String.class},
        ioDeclarationType, paths, offsets, lengths, segmentsNeeded, deadline);

    // Making assumption that all files are in the same namespace as the first
    List<RemoteLocation> locations =
        rpcServer.getLocationsForPath(paths.get(0), false, false);
    List<Long> tmp = (List<Long>) rpcClient.invokeSequential(locations, remoteMethod, List.class, null);
    LOG2.info("[RouterClientProtocol] response has [{}] indices", tmp.size());
    return tmp;
  }
  ...
  public List<Long> declareIOBlocks(String ioDeclarationType, List<List<LocatedBlock>> blockSets, long blockSetsNeeded, String deadline)
    ...
    RemoteMethod remoteMethod = new RemoteMethod("declareIOBlocks",
        new Class<?>[] {String.class, List.class, long.class, String.class},
        ioDeclarationType, blockSets, blockSetsNeeded, deadline);

    // Making the assumption that all blocks are in the same namespace.
    return (List<Long>) rpcClient.invokeSingle(blockSets.get(0).get(0).getBlock(), remoteMethod, List.class);
  }
