...
public class VolumeScanner extends Thread {
  ...
  private long scannedBytesTotalSum = 0; // value over all time.
  ...
  static class Statistics {
    ...
    long bytesScannedSinceRestart = 0;
    ...
    Statistics(Statistics other) {
      ...
      this.bytesScannedSinceRestart = other.bytesScannedSinceRestart;
    ...
    public String toString() {
      return new StringBuilder().
          ...
          append("bytesScannedSinceRestart=").append(bytesScannedSinceRestart).
  ...
  /**
   * Scan a list of blocks
   * @param blocks list of blocks to scan
   */
  public void scanBlocks(DeclareIOBlocksRequestProto request) throws IOException {
    int counter = 0;
    for (BlockSetProto blockSet : request.getBlockSetsList()) {
      counter += blockSet.getBlocksList().size();
    }
    LOG.info("[VolumeScanner] starting a scan of {} blocks", counter);
    int covered = 0;
    for (BlockSetProto blockSet : request.getBlockSetsList()) {
      for (HdfsProtos.LocatedBlockProto lb : blockSet.getBlocksList()) {
        // TODO: might need to rate limit here... although scanBlock should be throttling??
        ExtendedBlockProto eBlock = lb.getB();
        long bytesScanned = -1;
        boolean scanError = false;
        ExtendedBlock block = null;
        try {
          Block b = volume.getDataset().getStoredBlock(eBlock.getPoolId(), eBlock.getBlockId());
          covered++;
          if (b == null) {
            LOG.info("[VolumeScanner] skipping block {} ({} bytes) because replica not found in volume.", eBlock.getBlockId(), eBlock.getNumBytes());
            continue;
          }
          block = new ExtendedBlock(eBlock.getPoolId(), b);
          if (conf.skipRecentAccessed) {
            ...
            if (System.currentTimeMillis() - attr.lastAccessTime().
              to(TimeUnit.MILLISECONDS) < conf.scanPeriodMs) {
                LOG.info("[VolumeScanner] skipping block because recently accessed.");
                continue;
            }
          }
          LOG.info("[VolumeScanner] scanning {}", block);
          bytesScanned = scanBlock(block, conf.targetBytesPerSec);
          System.out.println("byteScanned: " + bytesScanned);
          if (bytesScanned >= 0) {
            scannedBytesSum += bytesScanned;
            scannedBytesTotalSum += bytesScanned;
          ...
          LOG.debug("Failed to get access time of block {}",
            block, ioe);
          LOG.info("[VolumeScanner] block not found, skipping scrubbing {} bytes", eBlock.getNumBytes());
        } catch (Exception e) {
          LOG.error("[VolumeScanner] caught unexpected error: ", e);
        } finally {
          synchronized (stats) {
            stats.bytesScannedInPastHour = scannedBytesSum;
            stats.bytesScannedSinceRestart = scannedBytesTotalSum;
            if (bytesScanned > 0) {
              stats.blocksScannedInCurrentPeriod++;
              stats.blocksScannedSinceRestart++;
              LOG.debug("After scanning block {}, blocks scanned: {}", block, stats.blocksScannedSinceRestart);
              LOG.info("After scanning block {}, blocks scanned: {}", block, stats.blocksScannedSinceRestart);
            }
            if (scanError) {
              stats.scanErrorsSinceRestart++;
            }
            ...
            if (curBlockIter == null) {
              ...
              LOG.debug("Blocks scanned so far: {} EOF: {}", stats.blocksScannedSinceRestart, stats.eof);
              stats.blockPoolPeriodEndsMs =
                      curBlockIter.getIterStartMs() + conf.scanPeriodMs;
            }
            LOG.info("[VolumeScanner] SCRUBBING-METRICS: total bytes scanned = {}", stats.bytesScannedSinceRestart);
          }
        }
      }
    }
    LOG.info("[VolumeScanner] finishing a scan of {} blocks, covered {}", counter, covered);
    return;
  }
  ...
  private long scanBlock(ExtendedBlock cblock, long bytesPerSec) {
    ...
    if (block == null) {
      LOG.info("[VolumeScanner] Block not found {}", cblock);
    ...
    try {
      ...
      LOG.debug("Block sender has been obtained for {}", block);
      throttler.setBandwidth(bytesPerSec);
      long bytesRead = blockSender.sendBlock(nullStream, null, throttler);
      LOG.debug("{} has read {}", block, bytesRead);
      resultHandler.handle(block, null);
      metrics.incrBlocksVerified();
      LOG.debug("Successfully scanned: {}", block);
      return bytesRead;
    } catch (IOException e) {
...
//        return 30000L;
      ...
      LOG.info("Block {} has been added to the queue", block.getBlockId());
      datanode.addToQueue(volume.getStorageID(), block);
      return 0L;
    } finally {
    }
  }
  ...
  public void run() {
    try {
      ...
      // Assuming one volume per datanode at the moment.
      ...
      LOG.info("[VolumeScanner] scanner waiting to get to {} blocks", conf.scrubbingDelayBlocks);
      long numBlocks = dvi != null ? dvi.getNumBlocks() : 0;
      while (numBlocks < conf.scrubbingDelayBlocks) {
        LOG.info("[VolumeScanner] volume has {} blocks, waiting to get to {}", numBlocks, conf.scrubbingDelayBlocks);
        sleep(30 * 1000);
      ...
      // Add delay before scanning for testing purposes
      LOG.info("[VolumeScanner] sleeping for {}s", conf.scrubbingDelay);
      sleep(conf.scrubbingDelay * 1000); // should this be wait?
      LOG.info("[VolumeScanner] done sleeping");
