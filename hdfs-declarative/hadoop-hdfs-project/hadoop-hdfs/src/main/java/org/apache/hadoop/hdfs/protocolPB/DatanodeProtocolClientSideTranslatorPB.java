...
public class DatanodeProtocolClientSideTranslatorPB implements
  ...
  public HeartbeatResponse sendHeartbeat(DatanodeRegistration registration,
    ...
    if (!queue.isEmpty()) {
      builder.setRequests(PBHelper.convertToNestedRequest(queue));
    }
    ...
    // .getResponse returns the I/O Planner response
    ConcurrentHashMap<String, DeclareIOBlocksRequestProto> map = PBHelper.convertNestedRequest(resp.getResponse());
    return new HeartbeatResponse(cmds, PBHelper.convert(resp.getHaStatus()),
        rollingUpdateStatus, resp.getFullBlockReportLeaseId(), map);
  }
