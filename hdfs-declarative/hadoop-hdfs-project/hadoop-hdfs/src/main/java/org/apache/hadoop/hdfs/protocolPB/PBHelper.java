...
import org.apache.hadoop.hdfs.protocol.proto.NamenodeProtocolProtos.AddBalancingRequestProto;
...
import org.apache.hadoop.hdfs.protocol.proto.DatanodeProtocolProtos.NestedRequest;
import org.apache.hadoop.hdfs.protocol.proto.DatanodeProtocolProtos.KeyValuePair;
...
public class PBHelper {
  ...
  public static AddBalancingRequestProto convert(NamenodeProtocolProtos.DeclareIOBlocksRequestProto req) {
    return AddBalancingRequestProto.newBuilder().setDatanodeUuid(req.getStorageID()).setNumBlocks(req.getBlockSetsNeeded()).build();
  }

  public static NamenodeProtocolProtos.BalancerHeartbeatRequestProto convert() {
    return NamenodeProtocolProtos.BalancerHeartbeatRequestProto.newBuilder().build();
  }
  ...
  public static NestedRequest convertToNestedRequest(ConcurrentHashMap<String, DeclareIOBlocksRequestProto> queue) {
    NestedRequest.Builder builder = NestedRequest.newBuilder();
    for (Map.Entry<String, DeclareIOBlocksRequestProto> entry : queue.entrySet()) {
      KeyValuePair.Builder keyValuePairBuilder = KeyValuePair.newBuilder();
      keyValuePairBuilder.setStorageID(entry.getKey());
      keyValuePairBuilder.setReq(entry.getValue());
      builder.addPair(keyValuePairBuilder.build());
    }
    return builder.build();
  }

  public static ConcurrentHashMap<String, DeclareIOBlocksRequestProto> convertNestedRequest(NestedRequest nr) {
    ConcurrentHashMap<String, DeclareIOBlocksRequestProto> resultMap = new ConcurrentHashMap<String, DeclareIOBlocksRequestProto>();

    for (KeyValuePair pair : nr.getPairList()) {
      resultMap.put(pair.getStorageID(), pair.getReq());
    }

    return resultMap;
  }

  public static ConcurrentHashMap<String, NamenodeProtocolProtos.DeclareIOBlocksRequestProto> convertIOPlannerBalancer(NamenodeProtocolProtos.NestedRequest nr) {
    ConcurrentHashMap<String, NamenodeProtocolProtos.DeclareIOBlocksRequestProto> resultMap = new ConcurrentHashMap<String, NamenodeProtocolProtos.DeclareIOBlocksRequestProto>();

    for (NamenodeProtocolProtos.KeyValuePair pair : nr.getPairList()) {
      resultMap.put(pair.getStorageID(), pair.getReq());
    }
  ...
  public static NamenodeProtocolProtos.NestedRequest convertIOPlannerResponseBalancer(
          ConcurrentHashMap<String, NamenodeProtocolProtos.DeclareIOBlocksRequestProto> queue) {
    NamenodeProtocolProtos.NestedRequest.Builder builder = NamenodeProtocolProtos.NestedRequest.newBuilder();
    for (Map.Entry<String, NamenodeProtocolProtos.DeclareIOBlocksRequestProto> entry : queue.entrySet()) {
      NamenodeProtocolProtos.KeyValuePair.Builder keyValuePairBuilder = NamenodeProtocolProtos.KeyValuePair.newBuilder();
      keyValuePairBuilder.setStorageID(entry.getKey());
      keyValuePairBuilder.setReq(entry.getValue());
      builder.addPair(keyValuePairBuilder.build());
    }
  ...
  public static BalancerHeartbeatResponse convert(NamenodeProtocolProtos.BalancerHeartbeatResponseProto res) {
    return new BalancerHeartbeatResponse(convertIOPlannerBalancer(res.getResponse()));
  }
