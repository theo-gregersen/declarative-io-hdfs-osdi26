...
public class DatanodeManager {
  ...
  private final Map<String, List<BlockECReconstructionInfo>> pendingECTasks;
  private final Map<String, List<BlockTargetPair>> pendingReplicationTasks;
  private final long reconstructionDeadline;
  ...
  DatanodeManager(final BlockManager blockManager, final Namesystem namesystem,
    ...
    this.pendingECTasks = new HashMap<>();
    this.pendingReplicationTasks = new HashMap<>();
    this.reconstructionDeadline = conf.getLong(DECLARATIVE_RECONSTRUCTION_DEADLINE_SECONDS_KEY,
        DECLARATIVE_RECONSTRUCTION_DEADLINE_SECONDS_DEFAULT) * 1000;
  }
  ...
  public DatanodeCommand[] handleHeartbeat(DatanodeRegistration nodeReg,
    ...
    // Should always hold because FSNamesystem is the only implementation; this code will need to be updated if
    // this assumption becomes invalid
    assert(namesystem instanceof FSNamesystem);
    FSNamesystem fs = (FSNamesystem) namesystem;
    IOPlanner plannerRef = fs.getIoPlannerRef();
    ...
    final BlockRecoveryCommand brCommand = getBlockRecoveryCommand(blockPoolId,
        nodeinfo);
    if (brCommand != null) {
      LOG.info("[DatanodeManager] block recovery triggered");
    ...
    if (totalBlocks > 0) {
      ...
      if (pendingList != null && !pendingList.isEmpty()) {
        ...
        // if there are replication tasks, declare them
        if (!pendingList.isEmpty()) {
          try {
            List<List<LocatedBlock>> blockSets = new ArrayList<>();
            List<LocatedBlock> blockSet = new ArrayList<>();
            for (BlockTargetPair blockTargetPair : pendingList) {
              // Copy block in case bytes get set to NO_ACK sometime later
              ExtendedBlock eb = new ExtendedBlock(blockPoolId, new Block(blockTargetPair.block));
              if (blockTargetPair.block.getNumBytes() == BlockCommand.NO_ACK) {
                LOG.info("[DatanodeManager] Replica block is deleted, so skip reconstruction");
                continue;
              }
              String[] storageId = {nodeinfo.getDatanodeUuid()};
              DatanodeInfo[] datanodeInfos = {nodeinfo};
              LocatedBlock lb = new LocatedBlock(eb, datanodeInfos, storageId, null);
              blockSet.add(lb);
            }
            if (blockSet.isEmpty()) {
              LOG.info("[DatanodeManager] Replica block set is empty, so skip");
            } else {
              blockSets.add(blockSet);
              UUID reqUuid = plannerRef.declarePhysical(
                  IOPlanner.DeclarationType.RECONSTRUCTION.name(),
                  blockSets,
                  1,
                  Long.toString(System.currentTimeMillis() + reconstructionDeadline),
                  "", "", nodeReg
              );
              pendingReplicationTasks.put(reqUuid.toString(), pendingList);
            }
      ...
      // If there are reconstruction tasks, declare them
      if (pendingECList != null && !pendingECList.isEmpty()) {
        try {
          List<List<LocatedBlock>> blockSets = new ArrayList<>();
          List<LocatedBlock> blockSet = new ArrayList<>();
          for (BlockECReconstructionInfo blockECReconstructionInfo : pendingECList) {
            if (blockECReconstructionInfo.getExtendedBlock().getNumBytes() == BlockCommand.NO_ACK) {
              LOG.info("[DatanodeManager] EC block is deleted, so skip reconstruction");
              continue;
            }
            DatanodeInfo[] dni = blockECReconstructionInfo.getSourceDnInfos();
            List<String> storageIds = new ArrayList<>();
            for (DatanodeInfo dn : dni) {
              storageIds.add(dn.getDatanodeUuid());
            }
            // Copy block in case bytes get set to NO_ACK sometime later
            LocatedBlock lb = new LocatedBlock(new ExtendedBlock(blockECReconstructionInfo.getExtendedBlock()),
              blockECReconstructionInfo.getSourceDnInfos(), storageIds.toArray(new String[0]), null);
            blockSet.add(lb);
          }
          if (blockSet.isEmpty()) {
            LOG.info("[DatanodeManager] EC block set is empty, so skip");
          } else {
            blockSets.add(blockSet);
            UUID reqUuid = plannerRef.declarePhysical(
                IOPlanner.DeclarationType.RECONSTRUCTION.name(),
            ...
            pendingECTasks.put(reqUuid.toString(), pendingECList);
          }
        } catch (Exception e) {
          throw new IOException(e);
        }
    ...
    List<BlockTargetPair> pendingList = new ArrayList<>();
    List<BlockECReconstructionInfo> pendingECList = new ArrayList<>();
    List<IOPlanner.Request> scheduledReconstructionRequests = plannerRef.pollDeclarationType(IOPlanner.DeclarationType.RECONSTRUCTION.name(), nodeReg);
    for (IOPlanner.Request req : scheduledReconstructionRequests) {
      String requestUuid = req.getUuid().toString();
      if (pendingECTasks.containsKey(requestUuid)) {
        pendingECList.addAll(pendingECTasks.get(requestUuid));
        pendingECTasks.remove(requestUuid);
      }
      if (pendingReplicationTasks.containsKey(requestUuid)) {
        pendingList.addAll(pendingReplicationTasks.get(requestUuid));
        pendingReplicationTasks.remove(requestUuid);
      }
    }
