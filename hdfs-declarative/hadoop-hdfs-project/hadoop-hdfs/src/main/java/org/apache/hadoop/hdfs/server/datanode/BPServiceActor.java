...
class BPServiceActor implements Runnable {
  ...
  private final long scrubbingDeadline;
  ...
  BPServiceActor(String serviceId, String nnId, InetSocketAddress nnAddr,
    ...
    scrubbingDeadline = conf.getLong(DECLARATIVE_SCRUBBING_DEADLINE_SECONDS_KEY,
        DECLARATIVE_SCRUBBING_DEADLINE_SECONDS_DEFAULT) * 1000;
  }
  ...
  HeartbeatResponse sendHeartBeat(boolean requestBlockReportLease)
    ...
    ConcurrentHashMap<String, DeclareIOBlocksRequestProto> queue = new ConcurrentHashMap<>();

      Map<String, Set<ExtendedBlock>> blockMap = dn.getAndResetQueue();
      ...
      String storageId = dn.getDatanodeUuid();
      for (Map.Entry<String, Set<ExtendedBlock>> blockSet : blockMap.entrySet()) {
        List<HdfsProtos.LocatedBlockProto> lbs = new ArrayList<>();
        for (ExtendedBlock eb : blockSet.getValue()) {
          lbs.add(HdfsProtos.LocatedBlockProto.newBuilder()
              .setB(PBHelperClient.convert(eb))
              .addStorageIDs(storageId)
              .setOffset(0)
              .setCorrupt(false)
              .setBlockToken(PBHelperClient.convert(new Token<>()))
              .build());
        }
        BlockSetProto bsp = BlockSetProto.newBuilder()
            .addAllBlocks(lbs)
            .build();
        // Storage ID here refers to the volume ID, not the datanode
        DeclareIOBlocksRequestProto req = DeclareIOBlocksRequestProto
                .newBuilder()
                .setIoDeclarationType(DeclarationType.SCRUBBING.name())
                .addBlockSets(bsp)
                .setBlockSetsNeeded(lbs.size())
                .setDeadline(Long.toString(System.currentTimeMillis() + scrubbingDeadline))
                .setStorageID(blockSet.getKey())
                .build();
        queue.put(blockSet.getKey(), req);
      }
    ...
    LOG.info("[BPServiceActor] RECONSTRUCTION-METRICS: EC tasks = {}, EC tasks failed = {}, EC tasks invalid = {}, "
    + "EC bytes read = {}, EC remote bytes read = {}, EC bytes written = {}, Rep blocks = {}, Rep bytes = {}, Rep blocks failed = {}",
      dn.getMetrics().getECReconstructionTasks(),
      dn.getMetrics().getECFailedReconstructionTasks(),
      dn.getMetrics().getECInvalidReconstructionTasks(),
      dn.getMetrics().getECReconstructionBytesRead(),
      dn.getMetrics().getECReconstructionRemoteBytesRead(),
      dn.getMetrics().getECReconstructionBytesWritten(),
      dn.getMetrics().getBlocksReplicated(),
      dn.getMetrics().getBytesReplicated(),
      dn.getMetrics().getFailedBlocksReplicated());
        ...
        if (sendHeartbeat) {
          ...
          if (!dn.areHeartbeatsDisabledForTests()) {
            ...
            dn.getMetrics().addHeartbeat(scheduler.monotonicNow() - startTime,
                getRpcMetricSuffix());
            if (!resp.isQueueEmpty()) {
              // Changing this to a separate thread so that it doesn't stall the heartbeat...
              final ConcurrentHashMap<String, DeclareIOBlocksRequestProto> toScrub = resp.getQueue();
              scrubbingExecutorService.submit(new Runnable() {
                @Override
                public void run() {
                  try {
                    dn.triggerScrub(toScrub);
                  } catch (IOException e) {
                    LOG.error("[BPServiceActor] caught error during triggerScrub", e);
                  }
                }
              });
            }
