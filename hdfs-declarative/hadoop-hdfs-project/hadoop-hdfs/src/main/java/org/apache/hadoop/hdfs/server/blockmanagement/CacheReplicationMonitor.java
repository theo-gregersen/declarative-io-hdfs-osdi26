...
public class CacheReplicationMonitor extends Thread implements Closeable {
  ...
  private String findReasonForNotCaching(CachedBlock cblock,
    ...
    } else if (cblock.getMark() != mark && !cacheManager.isDeclarativeCachedBlock(cblock)) {
  ...
  private void addNewPendingCached(final int neededCached,
    ...
    for (int i = 0; i < numReplicas; i++) {
      ...
      // If there's a set preference, use it (unless it's already being used)
      // This is also important for EC blocks since preferred = the datanode for that specific block (rather than group)
      if (cacheManager.getDeclarativePreferredDN().containsKey(cachedBlock)) {
        DatanodeDescriptor preferred = cacheManager.getDeclarativePreferredDN().get(cachedBlock);
        if (!pendingCached.contains(preferred) && !cached.contains(preferred) && !datanode.equals(preferred)) {
          continue;
        }
      }
      long pendingBytes = 0;
