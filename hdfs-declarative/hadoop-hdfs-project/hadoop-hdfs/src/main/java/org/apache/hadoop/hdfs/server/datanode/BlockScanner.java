...
import static org.apache.hadoop.hdfs.DFSConfigKeys.DECLARATIVE_SCRUBBING_PERIOD_SECONDS_KEY;
import static org.apache.hadoop.hdfs.DFSConfigKeys.DECLARATIVE_SCRUBBING_PERIOD_SECONDS_DEFAULT;
import static org.apache.hadoop.hdfs.DFSConfigKeys.DECLARATIVE_SCRUBBING_DELAY_SECONDS_KEY;
import static org.apache.hadoop.hdfs.DFSConfigKeys.DECLARATIVE_SCRUBBING_DELAY_SECONDS_DEFAULT;
import static org.apache.hadoop.hdfs.DFSConfigKeys.DECLARATIVE_SCRUBBING_DELAY_BLOCKS_KEY;
import static org.apache.hadoop.hdfs.DFSConfigKeys.DECLARATIVE_SCRUBBING_DELAY_BLOCKS_DEFAULT;
...
public class BlockScanner {
  ...
  static class Conf {
    ...
    final long scrubbingDelay;
    final long scrubbingDelayBlocks;
    ...
    private static long getConfiguredScanPeriodMs(Configuration conf) {
      ...
      long declarative = conf.getLong(DECLARATIVE_SCRUBBING_PERIOD_SECONDS_KEY,
          DECLARATIVE_SCRUBBING_PERIOD_SECONDS_DEFAULT);
      if (declarative != 0) {
        return TimeUnit.MILLISECONDS.convert(declarative, TimeUnit.SECONDS);
      }
    ...
    Conf(Configuration conf) {
      ...
      this.scrubbingDelay = conf.getLong(
          DECLARATIVE_SCRUBBING_DELAY_SECONDS_KEY,
          DECLARATIVE_SCRUBBING_DELAY_SECONDS_DEFAULT);
      this.scrubbingDelayBlocks = conf.getLong(
          DECLARATIVE_SCRUBBING_DELAY_BLOCKS_KEY,
          DECLARATIVE_SCRUBBING_DELAY_BLOCKS_DEFAULT);
  ...
  */
  public synchronized void addVolumeScanner(FsVolumeReference ref) {
    ...
    try {
      ...
      LOG.info("Adding scanner for volume {} (StorageID {})",
          volume, volume.getStorageID());
  ...
  /**
   * Triggers declarative scrubbing
   * @param queue The queue of blocks to scrub
   */
  public void triggerScrub(ConcurrentHashMap<String, DeclareIOBlocksRequestProto> queue) throws IOException {
    for (String StorageID: queue.keySet()) {
      LOG.info("[BlockScanner] Scrubbing triggered from declaration for storage id {} on datanode {}", StorageID, datanode.getDatanodeUuid());
      // Get the volume scanner, give it the blocks
      VolumeScanner scanner = scanners.get(StorageID);
      // If a scanner exists
      if (scanner != null) {
        LOG.info("[BlockScanner] found scanner");
        scanner.scanBlocks(queue.get(StorageID));
      } else {
        LOG.info("[BlockScanner] scanner is not found");
      }
    }
  }
