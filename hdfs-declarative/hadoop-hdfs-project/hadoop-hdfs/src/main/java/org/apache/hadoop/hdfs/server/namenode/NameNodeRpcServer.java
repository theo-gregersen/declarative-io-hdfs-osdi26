...
public class NameNodeRpcServer implements NamenodeProtocols {
  ...
  @Override
  public void addBalancingRequest(NamenodeProtocolProtos.DeclareIOBlocksRequestProto req) throws ServiceException {
    namesystem.addBalancerRequest(req);
  }

  @Override
  public BalancerHeartbeatResponse sendHeartbeat() throws IOException {
    return namesystem.sendBalancerHeartbeat();
  }
  ...
  @Override
  public void declareIO(String ioDeclarationType, String src, long offset, long length, String deadline)
      throws IOException {
    // Extra check to cap length at the file size
    HdfsFileStatus stat = getFileInfo(src);
    if (stat.getLen() < length) length = stat.getLen();
    // Check for transcoding
    src = namesystem.checkTranscoded(src);
    ...
    // Doing something to handle doby: LocatedBlock storageID typically stores storage storageID, not datanodeUuid
    // but for overlap's sake, we're writing datanodeUuid into the storageID array
    List<LocatedBlock> lbs = new ArrayList<>();
    for (LocatedBlock lb : locatedBlocks.getLocatedBlocks()) {
      List<String> storageIds = new ArrayList<>();
      List<StorageType> storageTypes = new ArrayList<>();
      List<DatanodeInfoWithStorage> locations = new ArrayList<>();
      // Either replica or RS (first locations are data blocks, see DFSStripedInputStream.java)
      // First location is first read target (?) (see DFSInputStream.java)
      int dataBlocks = 3;
      if (stat.getErasureCodingPolicy() != null) {
        dataBlocks = stat.getErasureCodingPolicy().getNumDataUnits();
      }
      int maxBlocks = lb.isStriped() ? dataBlocks : 1;
      for (int i = 0; i < maxBlocks && i < lb.getLocations().length; i++) {
        if (lb instanceof LocatedStripedBlock) {
          LocatedStripedBlock lsb = (LocatedStripedBlock) lb;
          if (lsb.getBlockIndices().length > i) {
            byte idxInGroup = lsb.getBlockIndices()[i];
            if (idxInGroup != i) {
              LOG.info("[NameNodeRpcServer] idx mismatch: {} {}", idxInGroup, i);
            }
          }
        }
        storageIds.add(lb.getLocations()[i].getDatanodeUuid());
        storageTypes.add(lb.getStorageTypes()[i]);
        locations.add(lb.getLocations()[i]);
      }
      lbs.add(new LocatedBlock(lb.getBlock(), locations.toArray(new DatanodeInfoWithStorage[0]),
          storageIds.toArray(new String[0]), storageTypes.toArray(new StorageType[0])));
    }
    blockSets.add(lbs);
    try {
      nn.declareIOBlocks(ioDeclarationType, blockSets, 1, deadline);
    } catch (Exception e) {
  ...
  @Override
  public List<Long> declareIOs(String ioDeclarationType, List<String> paths, List<Long> offsets, List<Long> lengths, long segmentsNeeded, String deadline)
      throws IOException {

    if (segmentsNeeded == 0 || segmentsNeeded > paths.size()) {
      throw new IOException("Segments needed must be > 0 and <= the number of segments declared");
    }

    List<List<LocatedBlock>> blockSets = new ArrayList<>();
    for (int i = 0; i < paths.size(); i++) {
      // Check for transcoding
      String src = namesystem.checkTranscoded(paths.get(i));
      // Extra check to cap length at the file size
      long length = lengths.get(i);
      HdfsFileStatus stat = getFileInfo(src);
      if (stat.getLen() < length) length = stat.getLen();
      LocatedBlocks locatedBlocks = namesystem.getBlockLocations(getClientMachine(), src, offsets.get(i), length);
      List<LocatedBlock> lbs = new ArrayList<>();
      for (LocatedBlock lb : locatedBlocks.getLocatedBlocks()) {
        List<String> storageIds = new ArrayList<>();
        List<StorageType> storageTypes = new ArrayList<>();
        ...
        int maxBlocks = lb.isStriped() ? dataBlocks : 1;
        for (int j = 0; j < maxBlocks && j < lb.getLocations().length; j++) {
          if (lb instanceof LocatedStripedBlock) {
            LocatedStripedBlock lsb = (LocatedStripedBlock) lb;
            if (lsb.getBlockIndices().length > j) {
              byte idxInGroup = lsb.getBlockIndices()[j];
              if (idxInGroup != j) {
                LOG.info("[NameNodeRpcServer] idx mismatch: {} {}", idxInGroup, j);
              }
            }
          }
          storageIds.add(lb.getLocations()[j].getDatanodeUuid());
          storageTypes.add(lb.getStorageTypes()[j]);
          locations.add(lb.getLocations()[j]);
        }
        lbs.add(new LocatedBlock(lb.getBlock(), locations.toArray(new DatanodeInfoWithStorage[0]),
            storageIds.toArray(new String[0]), storageTypes.toArray(new StorageType[0])));
      }
    ...
    try {
      return nn.declareIOBlocks(ioDeclarationType, blockSets, segmentsNeeded, deadline);
    } catch (Exception e) {
      throw new IOException(e);
    }
  }
  ...
  @Override
  public List<Long> declareIOBlocks(String ioDeclarationType, List<List<LocatedBlock>> blockSets, long blockSetsNeeded, String deadline)
      throws IOException {

    if (blockSetsNeeded == 0 || blockSetsNeeded > blockSets.size()) {
      throw new IOException("Block sets needed must be > 0 and <= the number of block sets declared");
    }

    try {
      return nn.declareIOBlocks(ioDeclarationType, blockSets, blockSetsNeeded, deadline);
    } catch (Exception e) {
      throw new IOException(e);
    }
  }
  ...
  @Override
  public String declareIOsAsync(String ioDeclarationType, List<String> paths, List<Long> offsets, List<Long> lengths,
                                long segmentsNeeded, String deadline, boolean allowPartialCompletion)
      throws IOException {
    ...
    for (int i = 0; i < paths.size(); i++) {
      // Check for transcoding
      String src = namesystem.checkTranscoded(paths.get(i));
      // Extra check to cap length at the file size (might not be necessary if getBlockLocations already handles)
      long length = lengths.get(i);
      HdfsFileStatus stat = getFileInfo(src);
      if (stat.getLen() < length) length = stat.getLen();
      LocatedBlocks locatedBlocks = namesystem.getBlockLocations(getClientMachine(), src, offsets.get(i), length);
    ...
    try {
      return nn.declareIOBlocksAsync(ioDeclarationType, blockSets, segmentsNeeded, deadline, allowPartialCompletion);
    } catch (Exception e) {
      throw new IOException(e);
    }
  }
  ...
  public List<Long> pollDeclaration(String requestUuid) {
    return nn.pollDeclaration(requestUuid);
  }
