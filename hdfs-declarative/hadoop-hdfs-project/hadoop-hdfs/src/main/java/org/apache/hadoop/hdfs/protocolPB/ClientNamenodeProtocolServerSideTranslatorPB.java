...
public class ClientNamenodeProtocolServerSideTranslatorPB implements
  ...
  @Override
  public DeclareIOResponseProto declareIO(
      RpcController controller, DeclareIORequestProto req)
      throws ServiceException {
    try {
      server.declareIO(
          req.getIoDeclarationType(),
          req.getPath(),
          req.getOffset(),
          req.getLength(),
          req.getDeadline());

      DeclareIOResponseProto.Builder builder = DeclareIOResponseProto.newBuilder();
  ...
  @Override
  public DeclareIOsResponseProto declareIOs(
      RpcController controller, DeclareIOsRequestProto req)
      throws ServiceException {
    try {

      // Converting SegmentProtos to lists; can create a class for this later
      List<String> paths = new ArrayList<>();
      List<Long> offsets = new ArrayList<>();
      List<Long> lengths = new ArrayList<>();
      for (SegmentProto segment : req.getSegmentsList()) {
        paths.add(segment.getPath());
        offsets.add(segment.getOffset());
        lengths.add(segment.getLength());
      }

      List<Long> segmentsSelected = server.declareIOs(
          req.getIoDeclarationType(),
          paths,
          offsets,
          lengths,
          req.getSegmentsNeeded(),
          req.getDeadline());

      DeclareIOsResponseProto.Builder builder = DeclareIOsResponseProto.newBuilder();
      builder.addAllSegmentsSelected(segmentsSelected);
  ...
  @Override
  public DeclareIOsAsyncResponseProto declareIOsAsync(
      RpcController controller, DeclareIOsAsyncRequestProto req)
      throws ServiceException {
    try {
      // Converting SegmentProtos to lists; can create a class for this later
      List<String> paths = new ArrayList<>();
      ...
      String requestUuid = server.declareIOsAsync(
          req.getIoDeclarationType(),
          paths,
          offsets,
          lengths,
          req.getSegmentsNeeded(),
          req.getDeadline(),
          req.getAllowPartialCompletion());

      DeclareIOsAsyncResponseProto.Builder builder = DeclareIOsAsyncResponseProto.newBuilder();
  ...
  @Override
  public DeclareIOsResponseProto pollDeclaration(
      RpcController controller, PollDeclarationRequestProto req)
      throws ServiceException {
    try {
      List<Long> segmentsSelected = server.pollDeclaration(
          req.getRequestUuid());
  ...
  @Override
  public DeclareIOBlocksResponseProto declareIOBlocks(
      RpcController controller, DeclareIOBlocksRequestProto req)
      throws ServiceException {
    try {
      List<List<LocatedBlock>> blockSets = new ArrayList<>();
      for (BlockSetProto blockSet : req.getBlockSetsList()) {
        blockSets.add(PBHelperClient.convertLocatedBlock(blockSet.getBlocksList()));
      }

      List<Long> blockSetsSelected = server.declareIOBlocks(
          req.getIoDeclarationType(),
          blockSets,
          req.getBlockSetsNeeded(),
          req.getDeadline());

      DeclareIOBlocksResponseProto.Builder builder = DeclareIOBlocksResponseProto.newBuilder();
      builder.addAllBlockSetsSelected(blockSetsSelected);
