...
public class CacheManager {
  ...
  private final ConcurrentHashMap<Long, Boolean> declarativeCachedBlocks;
  private final ConcurrentHashMap<CachedBlock, DatanodeDescriptor> declarativePreferredDN;
  private final ConcurrentHashMap<String, ConcurrentLinkedQueue<CachedBlock>> declarativeHeartbeatToCache;
  private final ConcurrentHashMap<String, ConcurrentLinkedQueue<CachedBlock>> declarativeHeartbeatToUncache;
  ...
  CacheManager(FSNamesystem namesystem, Configuration conf,
    ...
    this.declarativeCachedBlocks = new ConcurrentHashMap<>();
    this.declarativePreferredDN = new ConcurrentHashMap<>();
    this.declarativeHeartbeatToCache = new ConcurrentHashMap<>();
    this.declarativeHeartbeatToUncache = new ConcurrentHashMap<>();
  }

  public void cacheDeclarativeBlock(CachedBlock cb, DatanodeDescriptor dn) {
    if (dn == null) return;
    declarativeCachedBlocks.putIfAbsent(cb.getBlockId(), Boolean.TRUE);
    declarativePreferredDN.put(cb, dn);
    declarativeHeartbeatToCache
        .computeIfAbsent(dn.getDatanodeUuid(), k -> new ConcurrentLinkedQueue<>())
        .add(cb);
  }

  public void uncacheDeclarativeBlock(CachedBlock cb) {
    declarativeCachedBlocks.remove(cb.getBlockId());
    DatanodeDescriptor dn = declarativePreferredDN.remove(cb);
    if (dn != null) {
      declarativeHeartbeatToUncache
          .computeIfAbsent(dn.getDatanodeUuid(), k -> new ConcurrentLinkedQueue<>())
          .add(cb);
    }
  }

  public boolean isDeclarativeCachedBlock(CachedBlock cb) {
    return declarativeCachedBlocks.containsKey(cb.getBlockId());
  }

  public ConcurrentLinkedQueue<CachedBlock> getDeclarativeHeartbeatToCache(String datanodeUuid) {
    return declarativeHeartbeatToCache.get(datanodeUuid);
  }

  public ConcurrentLinkedQueue<CachedBlock> getDeclarativeHeartbeatToUncache(String datanodeUuid) {
    return declarativeHeartbeatToUncache.get(datanodeUuid);
  }

  public ConcurrentHashMap<CachedBlock, DatanodeDescriptor> getDeclarativePreferredDN() {
    return declarativePreferredDN;
  }
  ...
  private void processCacheReportImpl(final DatanodeDescriptor datanode,
    ...
    for (Iterator<Long> iter = blockIds.iterator(); iter.hasNext(); ) {
      ...
      // if it's not already there.  Skip declarative blocks: they are managed
      // via direct heartbeat commands and must not enter cachedBlocks, because
      // CRM would see replication=0 and immediately send DNA_UNCACHE.
      if (prevCachedBlock != null) {
        cachedBlock = prevCachedBlock;
      } else if (!declarativeCachedBlocks.containsKey(blockId)) {
