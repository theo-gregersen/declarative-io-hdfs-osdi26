...
public class LocatedBlock {
  ...
  // Can't import org.apache.hadoop.hdfs.server.common.HdfsServerConstants.MAX_BLOCKS_IN_GROUP?
  private final long BLOCK_GROUP_INDEX_MASK = 15;

  // Need these for IO Planner overlap calculations ---------------------------------
  // See Block.java and ExtendedBlock.java
  // Java hashset, hashmap use equals to resolve collision
  ...
  public boolean equals(Object o) {
    ...
    if (!(o instanceof LocatedBlock)) {
      return false;
    }
    LocatedBlock lb = (LocatedBlock) o;
    // Assuming not legacy (see BlockIdManager.java which can't be imported here for some reason)
    if (BlockType.fromBlockId(this.b.getBlockId()) != BlockType.fromBlockId(lb.b.getBlockId())) {
      return false;
    }
    // This check is necessary since storageID can be null
    boolean contains = false;
    if (this.storageIDs == null || lb.storageIDs == null) {
      contains = true;
    } else {
      for (String id : this.storageIDs) {
        for (String id2 : lb.storageIDs) {
          if (id.equals(id2)) {
            contains = true;
            break;
          }
    ...
    // Special doby check since blockId might be group-level or internal-level depending
    // on who is creating this located block
    // See Dispatcher.DBlock.getInternalBlock() and SequentialBlockGroupIdGenerator.java for reference
    if (BlockType.fromBlockId(this.b.getBlockId()) == BlockType.STRIPED) {
      // Special case for when blocks in same group are on same node? Seems to be a possibility?
      // Normal case (e.g. group-level ID)
      return (this.b.getBlockId() & ~BLOCK_GROUP_INDEX_MASK) == (lb.b.getBlockId() & ~BLOCK_GROUP_INDEX_MASK)
        && (this.b.getBlockPoolId() != null ? this.b.getBlockPoolId().equals(lb.b.getBlockPoolId()) : lb.b.getBlockPoolId() == null);
    } else {
      return this.b.equals(lb.b);
    }
  }

  @Override
  public int hashCode() { return BlockType.fromBlockId(b.getBlockId()) == BlockType.STRIPED ? 
	  Long.hashCode(b.getBlockId() & ~BLOCK_GROUP_INDEX_MASK) : Long.hashCode(b.getBlockId()); }
