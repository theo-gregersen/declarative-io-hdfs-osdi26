...
abstract class StripeReader {
  ...
  void readParityChunks(int num) throws IOException {
    // TODO: I believe this path is only called by client reads, not reconstruction reads
    // DFSClient.LOG.info("[Dec] Reading parity chunks: {}", num);
    for (int i = dataBlkNum, j = 0; i < dataBlkNum + parityBlkNum && j < num;
         i++) {
      if (alignedStripe.chunks[i] == null) {
        if (prepareParityChunk(i) && readChunk(targetBlocks[i], i)) {
          DFSClient.LOG.info("[Dec] Read parity chunk: {}", targetBlocks[i].getBlockSize() / dataBlkNum);
          j++;
        } else {
          alignedStripe.missingChunksNum++;
        }
