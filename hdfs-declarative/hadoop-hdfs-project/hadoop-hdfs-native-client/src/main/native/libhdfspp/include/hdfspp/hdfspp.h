...
class FileSystem {
 public:
  ...
  /**
   * Declare a single segment IO task to the IO Planner.
   * The handler is called when the IO Planner has determined the application should run its job.
   * This call blocks until the request is scheduled.
   **/
  virtual void DeclareIO(const std::string &declarationDesc,
  ...
  /**
   * Declare a multi-segment IO task to the IO Planner.
   * The handler is called when the IO Planner has determined the application should run its job.
   * This call blocks until the request is scheduled.
   **/
  virtual void DeclareIOs(const std::string &declarationDesc,
                          const std::vector<Segment> segments,
                          const uint64_t segmentsNeeded,
                          const std::string &deadline,
                          const std::function<void(const Status &, std::vector<uint64_t> &)> &handler) = 0;

  /**
   * Declare a multi-segment IO task to the IO Planner.
   * The handler is called when the request makes it to the planner and is assigned an ID.
   **/
  virtual void DeclareIOsAsync(const std::string &declarationDesc,
                               const std::vector<Segment> segments,
                               const uint64_t segmentsNeeded,
                               const std::string &deadline,
                               const bool allowPartialCompletion,
                               const std::function<void(const Status &, std::string &)> &handler) = 0;

  /**
   * Check if the declaration with requestUuid has been scheduled.
   * The handler is called when the scheduled requests queue has been checked.
   **/
  virtual void PollDeclaration(const std::string &requestUuid,
                               const std::function<void(const Status &, std::vector<uint64_t> &)> &handler) = 0;
