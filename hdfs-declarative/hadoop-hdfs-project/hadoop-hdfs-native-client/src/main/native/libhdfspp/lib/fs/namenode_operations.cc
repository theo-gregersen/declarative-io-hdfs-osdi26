...
void NameNodeOperations::DeclareIO(const std::string &declarationType,
  ...
  using ::hadoop::hdfs::DeclareIORequestProto;
  using ::hadoop::hdfs::DeclareIOResponseProto;

  LOG_INFO(kFileSystem, << "NameNodeOperations::DeclareIO(declarationType=" << declarationType
                        << ", path=" << path << ", offset=" << offset
                        << ", length=" << length <<") called");

  DeclareIORequestProto req;
  req.set_iodeclarationtype(declarationType);
  req.set_path(path);
  req.set_offset(offset);
  req.set_length(length);
  req.set_deadline(deadline);

  auto resp = std::make_shared<DeclareIOResponseProto>();

  // Don't seem to need RpcController here, probably due to service implementation.
  // https://protobuf.dev/reference/cpp/cpp-generated/#service
  namenode_.DeclareIO(&req, resp, [resp, handler](const Status &stat) {
    handler(stat);
  });

}

void NameNodeOperations::DeclareIOs(const std::string &declarationType,
                                    const std::vector<Segment> segments,
                                    const uint64_t segmentsNeeded,
                                    const std::string &deadline,
                                    const std::function<void(const Status &, std::vector<uint64_t> &)> &handler) {

  using ::hadoop::hdfs::SegmentProto;
  using ::hadoop::hdfs::DeclareIOsRequestProto;
  using ::hadoop::hdfs::DeclareIOsResponseProto;

  LOG_INFO(kFileSystem, << "NameNodeOperations::DeclareIOs(declarationType=" << declarationType
                        << ") called");

  DeclareIOsRequestProto req;
  req.set_iodeclarationtype(declarationType);
  req.set_segmentsneeded(segmentsNeeded);
  req.set_deadline(deadline);

  for (const Segment& src : segments) {
    SegmentProto *segment = req.add_segments();
    segment->set_path(src.path);
    segment->set_offset(src.offset);
    segment->set_length(src.length);
  }

  auto resp = std::make_shared<DeclareIOsResponseProto>();

  namenode_.DeclareIOs(&req, resp, [resp, handler](const Status &stat) {
    std::vector<uint64_t> segment_indices;
    if (stat.ok()) {
      int num_selected = resp->segmentsselected_size();
      for (int i = 0; i < num_selected; i++) {
        segment_indices.push_back(resp->segmentsselected(i));
      }
    }
    handler(stat, segment_indices);
  });
}

void NameNodeOperations::DeclareIOsAsync(const std::string &declarationType,
                                         const std::vector<Segment> segments,
                                         const uint64_t segmentsNeeded,
                                         const std::string &deadline,
                                         const bool allowPartialCompletion,
                                         const std::function<void(const Status &, std::string &)> &handler) {

  using ::hadoop::hdfs::SegmentProto;
  using ::hadoop::hdfs::DeclareIOsAsyncRequestProto;
  using ::hadoop::hdfs::DeclareIOsAsyncResponseProto;

  LOG_INFO(kFileSystem, << "NameNodeOperations::DeclareIOsAsync(declarationType=" << declarationType
                        << ") called");

  DeclareIOsAsyncRequestProto req;
  req.set_iodeclarationtype(declarationType);
  req.set_segmentsneeded(segmentsNeeded);
  req.set_deadline(deadline);
  req.set_allowpartialcompletion(allowPartialCompletion);
  ...
  auto resp = std::make_shared<DeclareIOsAsyncResponseProto>();

  namenode_.DeclareIOsAsync(&req, resp, [resp, handler](const Status &stat) {
    std::string uuid = resp->requestuuid();
    handler(stat, uuid);
  });
}

void NameNodeOperations::PollDeclaration(const std::string &requestUuid,
                                         const std::function<void(const Status &, std::vector<uint64_t> &)> &handler) {

  using ::hadoop::hdfs::PollDeclarationRequestProto;
  using ::hadoop::hdfs::DeclareIOsResponseProto;

  LOG_INFO(kFileSystem, << "NameNodeOperations::PollDeclaration(requestUuid=" << requestUuid
                        << ") called");

  PollDeclarationRequestProto req;
  req.set_requestuuid(requestUuid);

  auto resp = std::make_shared<DeclareIOsResponseProto>();

  namenode_.PollDeclaration(&req, resp, [resp, handler](const Status &stat) {
    std::vector<uint64_t> segment_indices;
    if (stat.ok()) {
      int num_selected = resp->segmentsselected_size();
      for (int i = 0; i < num_selected; i++) {
