...
void FileSystemImpl::DeclareIO(const std::string &declarationType,
                               const std::string &path,
  ...
  LOG_INFO(kFileSystem, << "FileSystemImpl::DeclareIO(declarationType=" << declarationType
                        << ", path=" << path << ", offset=" << offset
                        << ", length=" << length <<") called");

  nn_.DeclareIO(declarationType, path, offset, length, deadline, handler);
}

void FileSystemImpl::DeclareIOs(const std::string &declarationType,
                                const std::vector<Segment> segments,
                                const uint64_t segmentsNeeded,
                                const std::string &deadline,
                                const std::function<void(const Status &, std::vector<uint64_t> &)> &handler) {

  LOG_INFO(kFileSystem, << "FileSystemImpl::DeclareIOs(declarationType=" << declarationType
                        << ") called");

  nn_.DeclareIOs(declarationType, segments, segmentsNeeded, deadline, handler);
}

void FileSystemImpl::DeclareIOsAsync(const std::string &declarationType,
                                     const std::vector<Segment> segments,
                                     const uint64_t segmentsNeeded,
                                     const std::string &deadline,
  ...
  LOG_INFO(kFileSystem, << "FileSystemImpl::DeclareIOsAsync(declarationType=" << declarationType
                        << ") called");

  nn_.DeclareIOsAsync(declarationType, segments, segmentsNeeded, deadline, allowPartialCompletion, handler);
}

void FileSystemImpl::PollDeclaration(const std::string &requestUuid,
                                     const std::function<void(const Status &, std::vector<uint64_t> &)> &handler) {

  LOG_INFO(kFileSystem, << "FileSystemImpl::PollDeclaration(requestUuid=" << requestUuid
                        << ") called");

  nn_.PollDeclaration(requestUuid, handler);
}
